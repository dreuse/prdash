...
func TestRepoPickerDoesNotOfferAddForBareText(t *testing.T) {
	m := testModel(t, 200, 40, ViewBoard)
	m = openPicker(m)
	m = typePicker(m, "folha")

	for _, e := range m.repoEntries() {
		if e.add {
			t.Fatalf("%q is not owner/name and must not offer add", "folha")
		}
	}
}
...
func TestScopeSurvivesInTheHeaderAndState(t *testing.T) {
	...
	if !strings.Contains(stripANSI(m.View()), "other-service") {
...
func TestRepoPickerRendersCleanly(t *testing.T) {
	for _, scope := range []string{"", "vobys/other-service"} {
...
func TestRepoNamesDedupeIgnoringCase(t *testing.T) {
	m := testModel(t, 200, 40, ViewBoard)
	m.repos = []string{"vobys/pontoapp", "vobys/folha-spring-boot"}
	m.prs = append(m.prs, model.PullRequest{
		Repo: "vobys/PontoApp", Number: 5, Title: "t", Author: "a",
	...
	var seen int
	for _, n := range names {
		if strings.EqualFold(n, "vobys/pontoapp") {
			seen++
			if n != "vobys/PontoApp" {
	...
	if seen != 1 {
		t.Fatalf("pontoapp appears %d times in %v", seen, names)
	}
}

func TestTrackedReposAreCanonicalisedFromGithub(t *testing.T) {
	fixed := canonicalRepos(
		[]string{"vobys/pontoapp", "vobys/PONTOAPP", "vobys/other"},
		[]model.PullRequest{{Repo: "vobys/PontoApp"}},
	)
	if len(fixed) != 2 {
		t.Fatalf("duplicates were not merged: %v", fixed)
	}
	if fixed[0] != "vobys/PontoApp" {
...
func TestSettingsDedupeReposIgnoringCase(t *testing.T) {
	...
	s.Repos = []string{"vobys/pontoapp", "vobys/PontoApp", "vobys/other"}
...
func TestScopeMatchesIgnoringCase(t *testing.T) {
	m := testModel(t, 200, 40, ViewBoard)
	m.scope = "vobys/OTHER-SERVICE"
	if !m.inScope("vobys/other-service") {
...
func TestRepoPickerRemovesARepo(t *testing.T) {
	m := testModel(t, 200, 40, ViewBoard)
	m.settings.Repos = []string{"vobys/folha-spring-boot", "vobys/other-service"}
	m.scope = "vobys/other-service"
	...
	if containsFold(m.settings.Repos, "vobys/other-service") {
...
func TestRepoPickerKeepsTheLastRepo(t *testing.T) {
	m := testModel(t, 200, 40, ViewBoard)
	m.settings.Repos = []string{"vobys/folha-spring-boot"}
