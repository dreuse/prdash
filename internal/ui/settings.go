...
func (m Model) settingsFields() []settingsField {
	fields := []settingsField{
		...
		{section: "GENERAL", label: "Mouse", desc: "click to select, wheel to scroll - off frees text selection",
			kind: fieldToggle,
			get:  func(s config.Settings) string { return onOff(!s.DisableMouse) },
			set:  func(s *config.Settings, v string) { s.DisableMouse = v != "on" }},
