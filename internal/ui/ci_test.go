...
func TestCITableKeepsRunningRunsHoweverOld(t *testing.T) {
	...
	m.runs = append(m.runs, model.WorkflowRun{
		ID: 9100, Repo: "vobys/folha-spring-boot", Name: "CI", Branch: "ancient",
		Status:    "in_progress",
...
func TestCITableDropsFinishedRunsOutsideTheWindow(t *testing.T) {
	...
	m.runs = append(m.runs, model.WorkflowRun{
		ID: 9101, Repo: "vobys/folha-spring-boot", Name: "CI", Branch: "stale",
...
func TestCITableOrdersRunningBeforeFinished(t *testing.T) {
	m := testModel(t, 200, 60, ViewCI)
	m.runs = append(m.runs, model.WorkflowRun{
		ID: 9102, Repo: "vobys/folha-spring-boot", Name: "CI", Branch: "slow",
		Status:    "in_progress",
...
func TestCIFailuresOnlyFiltersTheTable(t *testing.T) {
	m := testModel(t, 200, 60, ViewCI)
	m.runs = append(m.runs, model.WorkflowRun{
		ID: 9003, Repo: "vobys/folha-spring-boot", Name: "CI", Branch: "master",
...
func TestCITableShowsRepoOnlyWhenUnscoped(t *testing.T) {
	...
	m.scope = "vobys/folha-spring-boot"
	m.rebuild()
	if strings.Contains(stripANSI(m.View()), "other-service") {
...
func TestCICursorFollowsItsRunAcrossARefresh(t *testing.T) {
	...
	m.runs = append([]model.WorkflowRun{{
		ID: 9200, Repo: "vobys/folha-spring-boot", Name: "CI", Branch: "brand-new",
		Status:    "in_progress",
		StartedAt: time.Now(),
		UpdatedAt: time.Now(),
	}}, m.runs...)
