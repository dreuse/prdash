...
func TestUrgencyScoring(t *testing.T) {
	...
	requested := base
	requested.RequestedReviewers = []string{"octodev"}
	...
	for _, tc := range tests {
		if got := Urgency(tc.pr, "octodev", 1); got != tc.want {
...
func TestUrgencyCombines(t *testing.T) {
	pr := PullRequest{
		...
		BehindBy:           40,
		RequestedReviewers: []string{"octodev"},
		Checks:             []Check{{State: CheckFailed}},
	}
	want := 40 + 30 + 20 + 4 + 6
	if got := Urgency(pr, "octodev", 1); got != want {
