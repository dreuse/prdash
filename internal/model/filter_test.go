...
func samplePR() PullRequest {
	return PullRequest{
		Repo:               "vobys/folha-spring-boot",
		Number:             12012,
		Title:              "SQL idempotente",
		Author:             "dreuse",
		HeadRef:            "feature/sql-idempotente",
		CreatedAt:          daysAgo(6),
		UpdatedAt:          daysAgo(1),
		BehindBy:           19,
		Labels:             []string{"database"},
		RequestedReviewers: []string{"alfoltran"},
...
func TestFilterMatch(t *testing.T) {
	pr := samplePR()
	ctx := FilterContext{Viewer: "alfoltran", Column: ColNeedsReview}
...
func TestStakeFor(t *testing.T) {
	pr := samplePR()
	pr.Reviews = []Review{{Login: "theorocha", State: ReviewApproved, SubmittedAt: daysAgo(2)}}
	...
	tests := []struct {
		...
		{"", StakeNone},
		{"dreuse", StakeAuthor},
		{"ALFOLTRAN", StakeRequested},
		{"theorocha", StakeApproved},
		{"nobody", StakeNone},
	}
	for _, tc := range tests {
...
func TestReviewStale(t *testing.T) {
	...
	pr.Reviews = []Review{{Login: "theorocha", State: ReviewApproved, SubmittedAt: daysAgo(3)}}

	if !pr.ReviewStale("theorocha") {
...
func TestFuzzyIsASubsequence(t *testing.T) {
	tests := []struct {
		...
		{"sql idempotente", "sqi", true},
		{"sql idempotente", "sqlidem", true},
		{"sql idempotente", "idem", true},
		{"sql idempotente", "qs", false},
		{"sql idempotente", "", true},
		{"", "x", false},
	}
	for _, tc := range tests {
...
func TestAssigneeFilter(t *testing.T) {
	ctx := FilterContext{Viewer: "theorocha", Column: ColNeedsReview}

	assigned := samplePR()
	assigned.Assignees = []string{"theorocha", "alfoltran"}
	...
	tests := []struct {
		...
		{"assignee:theorocha", assigned, true},
	...
	if !ParseFilter("assignee:theorocha").Valid() {
...
func TestFuzzyHandlesAccentedText(t *testing.T) {
	pr := samplePR()
	pr.Title = "Adição do campo Categoria"
	ctx := FilterContext{}

	for _, q := range []string{"adição", "Adição", "categoria", "adicat", "campo"} {
...
func TestFuzzyMatchesPerFieldNotAcrossThem(t *testing.T) {
	unrelated := samplePR()
	unrelated.Title = "Adição do campo Categoria"
	unrelated.HeadRef = "feat/categoria"
	unrelated.Author = "LevisFreitas1771"
	...
	if !ParseFilter("idempo").Match(target, ctx) {
		t.Fatal("idempo should match SQL idempotente")
	}
...
func TestNegatedTokens(t *testing.T) {
	ctx := FilterContext{Viewer: "dreuse", Column: ColNeedsReview}
...
func TestQuotedPhrases(t *testing.T) {
	ctx := FilterContext{}
	pr := samplePR()
	pr.Title = "SQL idempotente para folha"

	if !ParseFilter(`"idempotente para"`).Match(pr, ctx) {
		t.Error("a quoted phrase should match across the space")
	}
	if f := ParseFilter(`"idempotente para"`); len(f.Tokens) != 1 {
		t.Errorf("a quoted phrase is one token, got %d", len(f.Tokens))
	}
	if ParseFilter(`"para idempotente"`).Match(pr, ctx) {
